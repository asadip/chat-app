...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
    ...
    const {chats, setSelectedChat, them, setThem, user, navigate, 
  ...
  return (
    <div 
        className={`flex flex-col h-screen min-w-72 p-5 dark:bg-linear-to-b from-[#242124]/30
        to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-x1 transition-all 
        duration-500 max-md:absolute left-0 z-10 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      <img 
        src={them === 'dark' ? assets.logo_full : assets.logo_full_dark} 
        alt=""
        className='w-full max-w-40' 
      />

    {/* New Chats*/}
      <button onClick={createNewChat} className='flex justify-center items-center w-full py-1.5 mt-8
    ...
    {/* Search conversation*/}
      <div className='flex items-center gap-2 p-3 mt-3 border border-gray-500 dark:border-white/20 rounded-md'>
        <img 
            src={assets.search_icon} 
            className='w-3 not-dark:invert' 
            alt="" 
        />
        <input 
            ...
            className='text-xs placeholder:text-gray-500 outline-none'
        />
      </div>
        ...
        className='flex items-center gap-2 p-3 mt-4 border border-gray-300
         ...
         dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
            ...
            <div className='flex flex-col text-sm'>
                ...
                <p className='text-xs text-gray-500'>purchase credits to use ChatGPT</p>
            </div>
        </div>

        {/* Dark mode Toggle*/}
         <div className='flex items-center justify-between gap-2 p-3 mt-3 border
          border-gray-300 dark:border-white/15 rounded-md'>
            ...
            <label className='relative inline-flex cursor-pointer'>
                <input 
                    onChange={() => setThem(them === 'dark' ? 'light' : 'dark')} 
                    type="checkbox" 
                    className='sr-only peer' 
                    checked={them === 'dark'}
                />
                <div className='w-9 h-5 bg-gray-500 rounded-full peer-checked:bg-purple-600 
                transition-all'></div>
